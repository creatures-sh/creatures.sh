...
export const extractSlug = (
  ...
  switch (route) {
    ...
    case '/blog/[page]/[something]':
      return `${segments.page}-${segments.something}`
    default:
      return 'index'
  }
}

export const templatesMap: Record<string, TemplateNames> = {
  '/': 'default',
  '/about': 'default',
}

type TemplateNames = 'default'
...
"
            tw="text-[96px]"
          >
            creatures.dev${data?.page.slice(0, -1)}
          </h1>
          <p
        ...
        <div
          style="font-family: 'CalSans'; display: flex; gap: 24px;"
          tw="flex"
        >
          <div
...
export const templates = (
  ...
  const template = templatesMap[route]
  const templateFn = templateDefinitions[template]
  return templateFn
}
